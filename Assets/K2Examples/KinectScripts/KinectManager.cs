...
//			KinectInterop.ShutdownKinectSensor();
	...
	void OnGUI()
    {
        ...
        if (kinectInitialized)
		{
            //GUILayout.Box("inited");
            
            if (displayUserMap && !sensorData.color2DepthTexture &&
			   (computeUserMap != UserMapType.None && computeUserMap != UserMapType.RawUserDepth))
	        {
                //GUILayout.Box("display user map: " + displayUserMap + "\nsensorData.color: " + !sensorData.color2DepthTexture + "\ncomputeUserMap: " + computeUserMap);
                ...
                if (usersMapRect.width == 0 || usersMapRect.height == 0)
				{
                    ...
                    GUILayout.Box(cameraRect.ToString());
                    GUILayout.Box(displayWidth + " " + displayHeight);
					usersMapRect = new Rect(cameraRect.width /** viewPos.x*/ - displayWidth, cameraRect.height /** viewPos.y*/, displayWidth, -displayHeight);
				}
